    ...
    try {
       ...
       const { secure_url , public_id } = await cloudinary.uploader.upload(
        ...
        const user = await AuthorModel.create({
            name , email , password : hash , avatar : secure_url , avatarID : public_id 
        })
...
try {
    ...
    if(author){
    const pushed =    author?.chosen?.push(new mongoose.Types.ObjectId(category?._id))
...
export const findAuthors = async (req : Request , res : Response) =>{
    try {
        const author = await AuthorModel.find().sort({
            createdAt : -1
        }).populate("article").populate("requests")
        return res.status(200).json({
...
export const updateOneAuthor = async (req : Request , res: Response) =>{
    try {
        const {name} = req.body;
        const {authorID} = req.params
        const author = await AuthorModel.findByIdAndUpdate(authorID ,{name}, {new : true} )
